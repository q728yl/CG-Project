...
namespace Flame {
	...
	Flame::Flame()
	{
		...
		mUpdateShader = new Shader("Update.vs", "Update.fs",
			"Update.gs", varyings, 7);
		//设置TransformFeedback缓存能够记录的顶点的数据类型

		mRenderShader = new Shader("Render.vs", "Render.fs");
		//设置随机纹理
		InitRandomTexture(580);
		mSparkTexture = loadTexture("flame.bmp");
		mStartTexture = loadTexture("particle.bmp");
		mRenderShader->use();
